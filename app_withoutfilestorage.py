...
model.load_weights(
    'D:\Data Analytics for Business\Keras learning\dataset\dataset\cat-dog.hdf5')
...
def give_result(filename):
    im = cv2.imdecode(filename, cv2.IMREAD_COLOR)
...
def show_image():
    if request.method == 'POST':
        ...
        res = give_result(npimg)     # using give_result converting the input image to our desired size and predicting on the model
        print(res)
